...
export const Visit = ({
  ...
  const { selectedVisit, selectedShiva } = useSelector(
    (state: RootState) => state.shiva
  );
  ...
  const handleResizeFromTop = (ev: React.MouseEvent) =>
    handleMouseDown(ev, 'resize-from-top');
  const handleResizeFromBottom = (ev: React.MouseEvent) =>
    handleMouseDown(ev, 'resize-from-bottom');
      ...
      window.requestAnimationFrame(() => {
        ...
        if (node) {
          switch (interaction.type) {
            case 'drag':
              const topCoord =
                node.offsetTop + event.clientY - interactionRef.current.deltaY;
              if (topCoord <= 0) {
                node.style.top = '0px';
                break;
    ...
    if (attending) {
      dispatch(
        updateVisit({
          visitId: visit.id,
          partialVisit: {
            missingMourners: visit.missingMourners.filter(m => m !== mourner),
          },
        })
      );
    } else {
        ...
        case 'Visitor':
          return (
            <ToolTip
              ...
              top={`${node.offsetTop}px`}
              onHide={hideTip}
              backgroundColor={theme.colors.sauvignonLight}
            >
      ...
      >
        {mode === 'Edit' ? (
          <>
            ...
            {visit.visitors.length === 0 ? (
              <div
                className="gripper gripper-top"
                onMouseDown={handleResizeFromTop}
              ></div>
            ) : null}
            {visit.visitors.length === 0 ? (
              <div
                className="gripper gripper-bottom"
                onMouseDown={handleResizeFromBottom}
              ></div>
            ) : null}
          </>
        ) : null}
        <div>
          {mourners.length -
            (visit.missingMourners ? visit.missingMourners.length : 0)}{' '}
          Mourners
        </div>
