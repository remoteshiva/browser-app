...
const MournerRow = ({name, relationship, attending, role, onCheckChanged}: MournerRowProps) => {
  const [checked, setChecked] = useState(attending)
  const handleChange = () => {
    if(onCheckChanged)
      onCheckChanged(name+relationship, !checked)
    setChecked(!checked)
  }
  if(role==='Visitor' && !attending)
    return null
  return(
    <div className={`mourner ${role === 'Visitor' ? '' : 'editor'}`}>
      {role !== 'Visitor' ? <img src={checked ? CheckedIcon: UncheckedIcon} onClick={handleChange} alt='checkbox'/> : null }
...
interface Props extends Visit {
  ...
  onAddVisitor: ()=>void
  onToggleMournerParticipation: (mourner: MournerId, attending: boolean)=>void
}
