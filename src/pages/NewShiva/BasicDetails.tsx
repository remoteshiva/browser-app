...
const BasicDetails = ({newShiva, submit, selectStep}: StepProps<BasicDetailsProps>) => {
  ...
  return(
    <StepLayout
      ...
      step={1}
      submit={() => submit({...values}, Steps.VIDEO_CHAT_LINK)}
        ...
        <Row>
          <FixedColumn width={327}>
            <StyledForm>
              ...
              <label>
                Start date of shiva
                <input onChange={handleInputChange} type='date' name='startDate' value={values.startDate.format(dateFormat)} placeholder={dateFormat} required className='appearance-none block w-full bg-grey-lighter text-grey-darker border border-red rounded py-3 px-4 mb-3'/>
              </label>
              <label>
                Welcome message for visitors
                <textarea onChange={handleInputChange} name='message' value={values.message} placeholder='Type something here' className='appearance-none block w-full bg-grey-lighter text-grey-darker border border-red rounded py-3 px-4 mb-3'/>
              </label>
            </StyledForm>
          </FixedColumn>
          <FlexColumn>
